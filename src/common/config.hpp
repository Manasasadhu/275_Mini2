...
struct DataPartitioning {
    std::string strategy;
    std::vector<std::string> owned_counties;  // e.g. ["NY", "MN"] for Manhattan
};
...
public:
    static ProcessConfig loadConfig(const std::string& config_file) {
        ...
        // PARKING_DATA_PATH env var overrides config file
        const char* env_data_path = std::getenv("PARKING_DATA_PATH");
        if (env_data_path && std::string(env_data_path).length() > 0) {
            config.data_path = std::string(env_data_path);
            std::cout << "Using data path from PARKING_DATA_PATH env: " << config.data_path << std::endl;
        } else {
        ...
        config.data_partitioning.owned_counties = extractStringArray(content, "owned_counties");
