...
public:
    ParkingServiceImpl(const NodeConfig& config) : config_(config) {
        for (const auto& neighbor : config_.neighbors) {
            ...
            std::cout << "[" << timestamp() << "] Initialized neighbor " << neighbor
                      << " at " << neighbor_addr << std::endl;
        }
    }
...
private:
    ...
    grpc::Status SubmitQuery(grpc::ServerContext* context,
        ...
        // Build ForwardRequest for fanout + cache key
        ...
        if (cache_get(cache_key, cached_chunks)) {
            ...
            std::cout << "[" << timestamp() << "][" << config_.node_id
                      << "] CACHE HIT at gateway for key=" << cache_key
        ...
        std::cout << "[" << timestamp() << "][" << config_.node_id
                  << "] CACHE MISS at gateway, forwarding query" << std::endl;
        ...
        for (const auto& neighbor : config_.neighbors) {
            futures.push_back(std::async(std::launch::async, [this, neighbor, &forward_req]() {
                ...
                if (status.ok()) {
                    ...
                    std::cout << "[" << timestamp() << "][" << config_.node_id << "] Received "
                              << fwd_response.chunks_size() << " chunks from " << neighbor
                              << " (rpc_time=" << fwd_ms << " ms)" << std::endl;
                } else {
        ...
        response->set_request_id(req_id);
        std::cout << "[" << timestamp() << "][" << config_.node_id << "] Gateway stored "
                  << all_result_chunks.size() << " chunks for client pull (request_id=" << req_id
                  << ", total_time=" << gateway_ms << " ms)" << std::endl;
        return grpc::Status::OK;
    }
    ...
    grpc::Status ForwardQuery(grpc::ServerContext* context,
        ...
        if (is_cancelled(req_id)) {
            std::cout << "[" << timestamp() << "][" << config_.node_id << "] Query " << req_id
                      << " is cancelled, aborting" << std::endl;
        ...
        if (config_.shard && !data_file.empty()) {
            ...
            std::cout << "[" << timestamp() << "][" << config_.node_id
                      << "] Processing own shard (worker)" << std::endl;
            int chunk_size = request->chunk_size() > 0 ? request->chunk_size() : config_.chunk_size;
            auto chunks = process_query_on_shard(data_file, config_.shard, request, chunk_size);
            ...
            std::cout << "[" << timestamp() << "][" << config_.node_id << "] Own shard returned "
                      << chunks.size() << " chunks (scan_time=" << shard_ms << " ms)" << std::endl;
        }
        ...
        if (config_.role == "relay" || config_.role == "gateway") {
            ...
            // Rewrite from_node so downstream neighbors skip us, not the original sender
            parkingviolation::ForwardRequest outgoing_req(*request);
            outgoing_req.set_from_node(config_.node_id);

            for (const auto& neighbor : config_.neighbors) {
                if (neighbor == from_node) {
                    std::cout << "[" << timestamp() << "][" << config_.node_id << "] Skipping " << neighbor
                              << " (query came from here)" << std::endl;
                    continue;
                }
                std::cout << "[" << timestamp() << "][" << config_.node_id << "] Forwarding to neighbor "
                          << neighbor << std::endl;
                futures.push_back(std::async(std::launch::async, [this, neighbor, &outgoing_req]() {
                    ...
                    if (status.ok()) {
                        for (const auto& chunk : fwd_response.chunks()) chunks.push_back(chunk);
                        std::cout << "[" << timestamp() << "][" << config_.node_id << "] Collected "
                                  << fwd_response.chunks_size() << " chunks from " << neighbor
                                  << " (rpc_time=" << fwd_ms << " ms)" << std::endl;
                    } else {
                        std::cerr << "[" << timestamp() << "][" << config_.node_id << "] Error forwarding to "
                                  << neighbor << ": " << status.error_message()
                                  << " (" << fwd_ms << " ms)" << std::endl;
                    }
                    return chunks;
                }));
    ...
    grpc::Status CancelQuery(grpc::ServerContext* context,
        ...
        std::cout << "[" << timestamp() << "][" << config_.node_id << "] CancelQuery: cancelling request "
                  << req_id << std::endl;
